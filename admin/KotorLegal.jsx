...
Prices are shown in EUR, VAT included where applicable. Payments are processed by a regulated payment service provider (PCI DSS Level 1 certified). No card data is stored on our servers. The payment processor handles tokenisation, 3D Secure and SCA in accordance with PSD2.
...
- Payment metadata (brand, BIN, last 4 digits, issuing bank, country, risk score) — received from our payment processor; no full card number is stored
...
## 6. Sharing
- Our regulated payment processor (USA/EU, adequate safeguards under EU–US Data Privacy Framework); the processor's identity is available on request
...
## 4. Processing time
Approved refunds are returned to the original payment method within 5–10 business days. Processor handling time may add 2–5 additional business days depending on the issuing bank.

## 5. Chargebacks
Before initiating a chargeback with your bank, please contact us. Unjustified chargebacks may be contested through the payment processor's evidence process.
...
- Metapodaci plaćanja (brend kartice, BIN, posljednje 4 cifre, banka izdavalac, zemlja, risk score) — dobijeni od našeg procesora plaćanja; puni broj kartice se ne čuva
...
- Naš regulisani procesor plaćanja (SAD/EU, uz adekvatne zaštite iz EU–US Data Privacy Framework-a); identitet procesora dostupan na zahtjev
...
## 4. Vrijeme obrade
Odobreni povrati vraćaju se na isti metod plaćanja u roku od 5–10 radnih dana. Procesiranje kod procesora plaćanja može dodati 2–5 dodatnih radnih dana u zavisnosti od banke izdavaoca.

## 5. Chargeback
Prije iniciranja chargeback-a kod vaše banke, obratite nam se. Neosnovani chargeback-ovi se osporavaju kroz evidence proces našeg procesora plaćanja.
...
- Zahlungsmetadaten (Marke, BIN, letzte 4 Ziffern, Ausgabebank, Land, Risikobewertung) — vom Zahlungsabwickler bereitgestellt; vollständige Kartennummer wird nicht gespeichert
...
## 6. Weitergabe
- Unser regulierter Zahlungsabwickler (USA/EU, EU–US Data Privacy Framework); Identität auf Anfrage
...
## 4. Bearbeitungszeit
Genehmigte Rückerstattungen werden innerhalb von 5–10 Werktagen auf die ursprüngliche Zahlungsmethode zurückgebucht. Die Verarbeitung durch den Zahlungsabwickler kann je nach Bank weitere 2–5 Werktage hinzufügen.

## 5. Chargebacks
Vor einem Chargeback bei Ihrer Bank kontaktieren Sie bitte uns. Unbegründete Chargebacks werden über das Nachweisverfahren unseres Zahlungsabwicklers bestritten.
...
- Метаданные платежа (бренд, BIN, последние 4 цифры, банк-эмитент, страна, оценка риска) — от нашего обработчика платежей; полный номер карты не хранится
...
## 6. Передача
- Наш регулируемый обработчик платежей (США/ЕС, EU–US Data Privacy Framework); личность обработчика предоставляется по запросу
...
## 4. Сроки обработки
Одобренные возвраты возвращаются на исходный способ оплаты в течение 5–10 рабочих дней. Обработчик платежей может добавить 2–5 дополнительных дней.

## 5. Chargeback
Перед инициацией chargeback в банке — свяжитесь с нами. Необоснованные chargeback оспариваются через процесс нашего обработчика платежей.
...
- 姓名、邮箱、电话（可选）、账单地址（可选）
- 购买数据（类别、数量、金额、货币、语言）
- 支付元数据（卡组织、BIN、卡号后4位、发卡行、国家、风险评分）——来自我方支付处理商，不存储完整卡号
...
## 6. 共享
- 我方持牌支付处理商（美/欧，EU–US Data Privacy Framework），处理商身份可应请求告知
- 黑山税务局（ENU）
- 法律要求的主管机关
...
## 4. 处理时间
已批准的退款将在5–10个工作日内退回原支付方式。支付处理商处理时间可能额外增加2–5个工作日，取决于发卡行。

## 5. 拒付（Chargeback）
向银行发起拒付前，请先联系我们。无理由拒付将通过支付处理商的证据流程进行抗辩。
